...
export const editHistory = (history: string) => {
  let previousInfo: string=history1.slice(0, history1.search('HOLE CARDS') - 4);
  let holeCardInfo: string=history1.slice(history1.search('HOLE CARDS')+5, history1.search('HOLE CARDS') - 4)
  let flopInfo: string
  let turnInfo: string
  ...
  console.log(history1.search('HOLE CARDS'))
  console.log()

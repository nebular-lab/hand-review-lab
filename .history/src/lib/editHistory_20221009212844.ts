...
export const editHistory = (history: string) => {
  ...
  splitedhistory.forEach((line) => {
    ...
    if (line.includes('posts small blind')) {
      let sb = Number(line.slice(history1.search('HOLE CARDS')))
    }
  })
  playerCount /= 2

  let previousInfo: string = history1.slice(
    0,
    history1.search('HOLE CARDS') - 4
  )
  let holeCardInfo: string = history1.slice(
    history1.search('HOLE CARDS') + 15,
    history1.search('') - 4
  )
  let flopInfo: string = history1.slice(
    history1.search('FLOP') + 9,
    history1.search('TURN') - 4
  )
  let turnInfo: string = history1.slice(
    history1.search('TURN') + 9,
    history1.search('TURN') - 4
  )

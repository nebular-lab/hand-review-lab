  ...
  splitedhistory.forEach((line) => {
    ...
    if (line.includes(': folds')) {
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({ pos: player.pos, move: 'fold', size: 0,street:nowStreet,order:actions[nowStreet].length })
        }
      })
    }
    if (line.includes(': checks')) {
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({ pos: player.pos, move: 'check', size: 0 ,street:nowStreet,order:actions[nowStreet].length })
        }
      })
    }
    if (line.includes(': calls')) {
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({ pos: player.pos, move: 'call', size: size ,street:nowStreet,order:actions[nowStreet].length })
        }
      })
    }
    if (line.includes(': bets') && !line.includes('and is all-in')) {
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({ pos: player.pos, move: 'bet', size: size ,street:nowStreet,order:actions[nowStreet].length })
        }
      })
    }
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({
            ...
            move: 'raise',
            size: size,
            street:nowStreet,order:actions[nowStreet].length 
          })
        }
      })
    }
      ...
      players.forEach((player) => {
        if (player.name == name) {
          actions[nowStreet].push({
            pos: player.pos,
            move: 'allin',
            size: size,street:nowStreet,order:actions[nowStreet].length 
          })
        }
      })
    }

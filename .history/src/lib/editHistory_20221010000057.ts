...
`
PokerStars Zoom Hand #237673790529:  Hold'em No Limit ($0.01/$0.02) - 2022/07/30 0:31:52 ET
Table 'Halley' 6-max Seat #1 is the button
Seat 1: soraki12 ($2.69 in chips)
Seat 2: nebular77 ($2.02 in chips)
Seat 3: cintara ($0.61 in chips)
Seat 4: Germangirl87 ($1.57 in chips)
Seat 5: SylencY0 ($2.06 in chips)
Seat 6: brasileiro83 ($2.18 in chips)
nebular77: posts small blind $0.01
cintara: posts big blind $0.02
*** HOLE CARDS ***
Dealt to nebular77 [Ah 9h]
Germangirl87: calls $0.02
brasileiro83 is disconnected
SylencY0: folds
brasileiro83: folds
SylencY0 is disconnected
soraki12: folds
soraki12 is disconnected
nebular77: raises $0.13 to $0.15
cintara: folds
cintara is disconnected
Germangirl87: calls $0.13
*** FLOP *** [Td 7h 6s]
nebular77: bets $0.10
Germangirl87: calls $0.10
*** TURN *** [Td 7h 6s] [7d]
nebular77: checks
Germangirl87: checks
*** RIVER *** [Td 7h 6s 7d] [9s]
nebular77: checks
Germangirl87: checks
*** SHOW DOWN ***
nebular77: shows [Ah 9h] (two pair, Nines and Sevens)
Germangirl87: shows [Ts Kh] (two pair, Tens and Sevens)
Germangirl87 collected $0.50 from pot
*** SUMMARY ***
Total pot $0.52 | Rake $0.02
Board [Td 7h 6s 7d 9s]
Seat 1: soraki12 (button) folded before Flop (didn't bet)
Seat 2: nebular77 (small blind) showed [Ah 9h] and lost with two pair, Nines and Sevens
Seat 3: cintara (big blind) folded before Flop
Seat 4: Germangirl87 showed [Ts Kh] and won ($0.50) with two pair, Tens and Sevens
Seat 5: SylencY0 folded before Flop (didn't bet)
Seat 6: brasileiro83 folded before Flop (didn't bet)
...
export const editHistory = (history: string) => {
  ...
  const actions: { pos: string; move: string; size: number }[][] = [
    [],
    [],
    [],
    [],
  ...
  splitedhistory.forEach((line) => {
    if (line.includes('in chips)')) {
      playerCount++
      // TODO プレイヤー名に空白があった時
      playerName.push(line.split(' ').slice(2, -3))
    }
  })
  ...
  splitedhistory.forEach((line) => {
    ...
    if (line.includes(': bets') && !line.includes('and is all-in')) {
      ...
      actions[nowStreet].push({ pos: 'BB', move: 'bet', size: size })
    }
    if (line.includes(': raises') && !line.includes('and is all-in')) {
      const size = Number(line.split(' ').slice(-1)[0].replace('$', ''))
      actions[nowStreet].push({ pos: 'BB', move: 'raise', size: size })
    }
    if (line.includes('and is all-in')) {
      ...
      actions[nowStreet].push({ pos: 'BB', move: 'allin', size: size })
    }
  })
